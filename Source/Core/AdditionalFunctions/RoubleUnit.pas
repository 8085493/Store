...
begin {Возвращает число прописью с рублями и копейками}
  ...
  begin
    ...
    kop := Copy(Result, Length(Result) - 1, 3);
   //   Result := NumToStr(rub) + ' рубл'+ruble[Char((ending(rub)))] ;
    Result := NumToStr(rub) + ' белорусских рубл' + ruble[StrToINT(ending(rub))]
      + ' ' + NumToStr(kop) + ' копе' + Kopeek[StrToINT(ending(kop))];
